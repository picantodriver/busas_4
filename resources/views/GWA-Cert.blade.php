...
<body>
    <div class="certificate">
        ...
        <div class="content">
            ...
            @endphp
            has graduated with the degree of <strong>{{ $student->records()->first()?->curricula?->programs?->program_name }} ({{ $student->records()->first()?->curricula?->programs?->program_abbreviation}}){{($honor = $student->graduationInfos->first()?->latin_honor) ?", $honor" : '' }}</strong> <!--</strong>from {{ $collegeName }}, {{ $campus }}-->from Bicol University, Legazpi City on {{ date('F j, Y', strtotime($student->graduationInfos->first()->graduation_date)) }}
