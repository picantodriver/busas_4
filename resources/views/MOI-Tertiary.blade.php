...
<body>
    <div class="certificate">
        ...
        <div class="content">
            ...
            @endphp
            has graduated with the degree of <strong>{{ $student->records()->first()?->curricula?->programs?->program_name }} ({{ $student->records()->first()?->curricula?->programs?->program_abbreviation}}){{($honor = $student->graduationInfos->first()?->latin_honor) ?", $honor" : '' }} </strong>from <!--{{ $collegeName }}, {{ $campus }}--> Bicol University, Legazpi City on {{ date('F j, Y', strtotime($student->graduationInfos->first()->graduation_date)) }}
            per Referendum No. {{ $student->graduationInfos->first()->board_approval }}, of the Board of Regents, Bicol University<!--, Legazpi City-->.
        </div>
        <div class="content">
            It is further certified that <strong>ENGLISH LANGUAGE</strong> is the medium of instruction
            in <!--all--> the Tertiary Level courses of Bicol University<!--except for Filipino and foreign language
            subjects-->.
        </div>
        <div class="content-issued">
            Issued this {{ now()->format('j') }}<sup>{{ now()->format('S') }}</sup> day of {{ now()->format('F, Y') }} upon the request of the interested<!--{{ $student->sex == 'M' ? 'Mr.' : 'Ms.' }} {{($student->last_name) }}--> party for
            reference purposes.
        </div>
